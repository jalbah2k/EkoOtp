...
public partial class Controls_ContentSlider_ContentSlider : System.Web.UI.UserControl
{
    #region properties
    ...
    private bool _adaptiveHeight = false;
    public bool adaptiveHeight
    {
        get
        {
            return _adaptiveHeight;
        }
        set
        {
    ...
    public Controls_ContentSlider_ContentSlider()
    {
        GalleryId = "1";
    }
    public Controls_ContentSlider_ContentSlider(string par)
    {
        GalleryId = par;
    }
    protected void Page_Init(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                string sqlstr = "select * from ContentSliderGallery where id=@GalleryId";
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                {
                    //litTitle.Text = string.Format("<div class=\"content-slider-title\"><h2>{0}</h2></div>", dt.Rows[0]["Title"]);
                    litTitle.Text = string.Format("<h2>{0}</h2>", dt.Rows[0]["Title"]);
                }
                else
                {
                    litTitle.Text = "&nbsp;";
                ...
                _width = Convert.ToInt32(dt.Rows[0]["width"]);
                _height = Convert.ToInt32(dt.Rows[0]["height"]);
                _adaptiveHeight = (bool)dt.Rows[0]["adaptiveHeight"];
    ...
    private void BindData()
    {
        ...
        {
            string sqlstr = "select * from ContentSlider where GalleryId=@GalleryId order by priority";
        ...
        if (dt.Rows.Count > 0)
        {
            ...
            this.repContentSlider.DataSource = dv;
            this.repContentSlider.DataBind();
        }
    }

    protected void repContentSlider_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        //if (drv["URL"].ToString() != "")
        //    litContent.Text = "<a href='" + drv["URL"].ToString() + "' target='" + drv["Target"].ToString() + "'>" + drv["Html"].ToString() + "</a>";
        //else
        //    litContent.Text = drv["Html"].ToString();

        StringBuilder sb = new StringBuilder();

        sb.Append("<div class=\"content-slider-item\" style=\"width:");
        sb.Append(Width); sb.Append("px;");
        if (!adaptiveHeight)
        {
            sb.Append(" height:");
            sb.Append(Height);
        ...
        if (drv["URL"].ToString() != "" && !drv["html"].ToString().Contains("Responsive-Video"))
        {
            sb.Append("<a href='");
