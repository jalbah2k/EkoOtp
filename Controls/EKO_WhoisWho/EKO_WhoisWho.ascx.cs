...
public partial class EKO_WhoisWho : System.Web.UI.UserControl
{
    ...
    private void EnsureOrganizationDropdown()
    {
        ...
        try
        {
            foreach (var org in LoadOrganizations().OrderBy(o => o.Name, StringComparer.OrdinalIgnoreCase))
            {
                ddlOrganization.Items.Add(new ListItem(org.Name, org.Id));
            }
        }
        catch
        {
    ...
    {
        var orgById = new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase);
        ddlOrganization.Items.Clear();
        ddlOrganization.Items.Add(new ListItem("All Organizations", ""));
    ...
    private List<WhoisWhoOrganization> LoadOrganizations()
    {
        ...
        using (var conn = new SqlConnection(ConfigurationManager.AppSettings["CMServer"]))
        using (var dapt = new SqlDataAdapter("[eko].[Organizations_Sel_New]", conn))
        {
            ...
            {
                if (!IsActiveEkoOrEkoPncaOrganization(row, true))
                {
                    continue;
                }
                AddOrg(list, row);
            ...
            if (list.Count == 0)
            {
                foreach (DataRow row in ds.Tables[0].Rows)
                {
                    if (!IsActiveEkoOrEkoPncaOrganization(row, false))
                    {
                        continue;
                    }
                    AddOrg(list, row);
    ...
    private static void AddOrg(List<WhoisWhoOrganization> list, DataRow row)
    {
        var id = Col(row, "id", "ID", "OrganizationId");
        var name = Col(row, "name", "Name", "Organization", "OrganizationName");
        if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(name))
        {
            return;
        }

        if (list.Exists(o => o.Id == id.Trim()))
        {
            return;
        }

        list.Add(new WhoisWhoOrganization { Id = id.Trim(), Name = name.Trim() });
    }

    private static bool IsActiveEkoOrEkoPncaOrganization(DataRow row, bool applyTypeFilter)
    {
        var table = row.Table;
        if (HasColumn(table, "active") && !IsTruthy(row["active"]))
        {
            return false;
        }
        if (HasColumn(table, "deleted") && IsTruthy(row["deleted"]))
        {
            return false;
        }
        ...
        // EKO = 1, Building Capacity = 2, PNCA = 3, EKO-PNCA = 4
        if (HasColumn(table, "Type") || HasColumn(table, "OrganizationType") || HasColumn(table, "type"))
        {
            var typeValue = Col(row, "Type", "OrganizationType", "type").Trim();
            int typeId;
            if (int.TryParse(typeValue, out typeId))
            {
                return typeId == 1 || typeId == 4;
            }

            var normalized = NormalizeKey(typeValue);
            return normalized == "eko" || normalized == "ekopnca" || normalized == "eko-pnca";
        }
    ...
    private List<WhoisWhoMember> LoadMembers(Dictionary<string, string> orgById)
    {
        ...
        DataTable memberTable = FillTable(
            @"SELECT
                ...
                CASE WHEN m.ProfilePhoto IS NULL THEN 0 ELSE 1 END AS HasPhoto
              FROM eko.Members m
    ...
    private Dictionary<string, List<string>> LoadCommitteesByUserId()
    {
        ...
        DataTable groups = FillTable(
            "SELECT id, name, yaf_GroupName, ForumRoleId, ShowOnRegistration, supergroup FROM Groups",
            "SELECT id, name FROM Groups");

        var excludedIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        var committeeGroupIds = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        foreach (DataRow row in groups.Rows)
        {
            var id = Col(row, "id", "ID");
            var name = Col(row, "name", "Name", "yaf_GroupName");
            if (IsExcludedRoleName(name) || IsExcludedRoleName(Col(row, "yaf_GroupName")))
            {
                excludedIds.Add(id);
                continue;
            }

            var hasForumRole = groups.Columns.Contains("ForumRoleId")
                && row["ForumRoleId"] != DBNull.Value
                && !string.IsNullOrWhiteSpace(Convert.ToString(row["ForumRoleId"]));
            var showOnReg = groups.Columns.Contains("ShowOnRegistration") && IsTruthy(row["ShowOnRegistration"]);
            var looksLikeCommittee = name.IndexOf("committee", StringComparison.OrdinalIgnoreCase) >= 0
                || name.IndexOf("working group", StringComparison.OrdinalIgnoreCase) >= 0
                || name.IndexOf("knowledge network", StringComparison.OrdinalIgnoreCase) >= 0
                || name.IndexOf("communities of practice", StringComparison.OrdinalIgnoreCase) >= 0;

            if (hasForumRole || showOnReg || looksLikeCommittee)
            {
                committeeGroupIds.Add(id);
            }
        }

        DataTable access = FillTable(
            @"SELECT uga.User_id, g.id, g.name, g.yaf_GroupName, g.ForumRoleId, g.ShowOnRegistration
              FROM Users_Groups_Access uga
              INNER JOIN Groups g ON g.id = uga.Group_id",
            @"SELECT uga.User_id, g.id, g.name
              FROM Users_Groups_Access uga
              INNER JOIN Groups g ON g.id = uga.Group_id");

        foreach (DataRow row in access.Rows)
        {
            var groupId = Col(row, "id", "Group_id");
            if (excludedIds.Contains(groupId))
            {
                continue;
            }

            if (committeeGroupIds.Count > 0 && !committeeGroupIds.Contains(groupId))
            {
                continue;
            }

            var userId = Col(row, "User_id", "userid");
            var groupName = Col(row, "name", "Name", "yaf_GroupName");
            ...
            List<string> names;
            if (!result.TryGetValue(userId, out names))
            {
        ...
        foreach (var pair in result.ToList())
        {
