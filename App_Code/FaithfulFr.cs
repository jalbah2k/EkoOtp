...
{
    public static class FaithfulFr
    {
        private static IEnumerable<string> words(string text)
        {
              return Regex.Matches(text.ToLower(), "[a-z-àâçéèêîïoôœuû]+")
                        .Cast<Match>()
                        .Select(m => m.Value);
        }
        ...
        private const string alphabet = "aàâbcdeéèêfghiîïjklmnoôœpqrstuûvwxyz";
        ...
        private static string[] mycorrect(string word)
        {
            var candidates =
                new[] { known(new[] {word}),
                    known(edits1(word)),
            ...
            IOrderedEnumerable<string> lista = candidates.OrderByDescending(c => NWORDS(c) ?? 1);

            return lista.ToArray();
        }

        public static void SetDictionary(string text)
        {
            NWORDS = train(words(text));
        }

        public static string GetCorrectedWord(string word)
        {
            return correct(word);
        }

        public static string[] GetCorrectedWords(string word)
        {
            return mycorrect(word);
        }
