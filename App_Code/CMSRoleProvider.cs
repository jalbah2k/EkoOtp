...
/// <summary>
/// Summary description for CMSRoleProvider
/// </summary>
public class CMSRoleProvider : RoleProvider
{
            ...
            try
            {
                ...
                for (int iterU = 0; iterU < usernames.Length; iterU++)
                {
                    ...
                    for (int iterR = 0; iterR < roleNames.Length; iterR++)
                    {
                        ...
                        //                           connection);
                        command = new SqlCommand(@"SELECT User_id FROM Users_Groups_Access WHERE User_id = @UserId AND Group_id = @RoleId",
                                                   connection);
                ...
                for (int iterU = 0; iterU < usernames.Length; iterU++)
                {
                    for (int iterR = 0; iterR < roleNames.Length; iterR++)
                    {
                        ...
                        //                            connection);
                        command = new SqlCommand(@"INSERT INTO Users_Groups_Access (User_id, Group_id, Access_Level) VALUES(@UserId, @RoleId, 1)",
                                                    connection);
                        command.Parameters.Add(new SqlParameter("@UserId", userIds[iterU]));
                        command.Parameters.Add(new SqlParameter("@RoleId", roleIds[iterR]));
    ...
    public override void CreateRole(string roleName)
    {
        ...
        try
        {
            try
            {
                ...
                command = new SqlCommand(@"INSERT INTO Groups (name) VALUES (@RName)", connection);
        ...
        try
        {
            try
            {
                ...
                if (throwOnPopulatedRole)
                {
                    ...
                    //                    connection);
                    command = new SqlCommand(@"SELECT COUNT(*) " +
                                           @"FROM Users_Groups_Access ur, Users u " +
                                           @"WHERE ur.Group_id = @RoleId AND ur.User_id = u.id",
                                        connection);
                ...
                command = new SqlCommand(@"DELETE FROM Groups WHERE id = @RoleId", connection);
                command.Parameters.Add(new SqlParameter("@RoleId", roleId));
                int returnValue = command.ExecuteNonQuery();
                command = new SqlCommand("COMMIT TRANSACTION", connection);
                command.ExecuteNonQuery();
    ...
    public override string[] FindUsersInRole(string roleName, string usernameToMatch)
    {
        ...
        try
        {
            try
            {
                ...
                command = new SqlCommand(@"SELECT UserName " +
                                           @"FROM Users_Groups_Access ur, Users u " +
                                           @"WHERE ur.Group_id = @RoleId AND ur.User_id = u.id AND u.UserName LIKE @UserNameToMatch " +
                                           @"ORDER BY UserName", connection);
...
#if !ROLE_ID
                command = new SqlCommand(@"SELECT name FROM Groups ORDER BY name", connection);
                reader = command.ExecuteReader(CommandBehavior.SequentialAccess);
                while (reader.Read())
                    sc.Add(reader.GetString(0));
#else
                command = new SqlCommand(@"SELECT id FROM Groups ORDER BY name", connection);
                reader = command.ExecuteReader(CommandBehavior.SequentialAccess);
                while (reader.Read())
                    sc.Add("g" + reader.GetInt32(0).ToString());
#endif
    ...
    public override string[] GetRolesForUser(string username)
    {
        ...
        try
        {
            try
            {
                ...
                ////                           connection);
                ///
                command = new SqlCommand(@"SELECT r.name FROM Users_Groups_Access ur, Groups r " +
                                            @"WHERE ur.User_id = @UserId AND ur.Group_id = r.id " +
                                            @"ORDER BY r.name",
                                            connection);
                ...
                command = new SqlCommand(@"SELECT r.id FROM Users_Groups_Access ur, Groups r " +
                                            @"WHERE ur.User_id = @UserId AND ur.Group_id = r.id " +
                                            @"ORDER BY r.name",
                                           connection);
                command.Parameters.Add(new SqlParameter("@UserId", userId));
    ...
    public override string[] GetUsersInRole(string roleName)
    {
        ...
        try
        {
            try
            {
                ...
                //                           @"ORDER BY UserName",
                //                        connection);
                command = new SqlCommand(@"SELECT u.UserName " +
                                           @"FROM Users_Groups_Access ur, Users u " +
                                           @"WHERE ur.Group_id = @RoleId AND ur.User_id = u.id " +
                                           @"ORDER BY u.UserName",
                                        connection);
    ...
    public override bool IsUserInRole(string username, string roleName)
    {
        ...
        try
        {
            try
            {
                ...
                command = new SqlCommand(@"SELECT User_id FROM Users_Groups_Access WHERE User_id = @UserId AND Group_id = @RoleId", connection);
                command.Parameters.Add(new SqlParameter("@UserId", userId));
                command.Parameters.Add(new SqlParameter("@RoleId", roleId));
            ...
            try
            {
                ...
                for (int iterU = 0; iterU < usernames.Length; iterU++)
                {
                    for (int iterR = 0; iterR < roleNames.Length; iterR++)
                    {
                        ...
                        //                            connection);
                        command = new SqlCommand(@"DELETE FROM Users_Groups_Access WHERE User_id = @UserId AND Group_id = @RoleId",
                                                    connection);
                        command.Parameters.Add(new SqlParameter("@UserId", userIds[iterU]));
                        command.Parameters.Add(new SqlParameter("@RoleId", roleIds[iterR]));
                        if (command.ExecuteNonQuery() != 1)
    ...
    private int GetRoleId(SqlConnection connection, int appId, string roleName)
    {
        ...
        //                           connection);
        command = new SqlCommand(@"SELECT id FROM Groups WHERE name = @RoleName",
                                   connection);
